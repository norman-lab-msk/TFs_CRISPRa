...
def ks_de(pop, key, control_cells, genes=None, cells=None, normalized=False, normalized_name=None, n_jobs=1, alpha=0.05, multi_method='fdr_by', **kwargs):
    ...
    control_matrix = pop.where(cells=control_cells, genes=genes, normalized=normalized, normalized_name=normalized_name, **kwargs)
    print("{0} control cells".format(control_matrix.shape[0]))
    subpops = pop.groupby(key, cells=cells, genes=genes, normalized=normalized, normalized_name=normalized_name, **kwargs)
...
def ttest(pop, key, control_cells, genes=None, cells=None, normalized=False, normalized_name=None, n_jobs=1, alpha=0.05, multi_method='fdr_by', **kwargs):
    """Look for differential gene expression relative to a control population based on t-test.
    The function will do the test for each subpopulation defined by a category in pop.cells.
    ...
    Returns:
        t_matrix: matrix of test statistics for each gene in each subpopulation against the control population
        p_matrix: p-values
        adj_p_matrix: p-values corrected for multiple hypothesis testing
    ...
    out = Parallel(n_jobs=n_jobs, verbose=10)(delayed(_ttest_compare_pops)(subpop, control_matrix, name) for name, subpop in subpops)
    
    Ts, ps = zip(*out)
    Ts = pd.DataFrame(list(Ts)).T
    ps = pd.DataFrame(list(ps)).T
...
def ad_de(pop, key, control_cells, genes=None, cells=None, normalized=False, normalized_name=None, n_jobs=1, alpha=0.05, multi_method='fdr_by', **kwargs):
...
def _ttest_compare_pops(first_pop_matrix, second_pop_matrix, name=None):
    """Helper function used to execute ttest. See ttest.
    """
    t_stats = dict()
    p_stats = dict()
    for gene_id in first_pop_matrix.columns:
        t, p = ttest_ind(first_pop_matrix[gene_id], \
                         second_pop_matrix[gene_id])
        t_stats[gene_id] = t
        p_stats[gene_id] = p
    return pd.Series(t_stats, name=name), pd.Series(p_stats, name=name)
...
def _prep_X_y(pop, key, cells=None, genes=None, normalized=True, normalized_name=None, feature_table=None, ignore=None, verbose=False, **kwargs):
    ...
    if feature_table is None:
        matrix = pop.where(cells=cells, genes=genes, normalized=normalized, normalized_name=normalized_name, **kwargs)
    else:
...
def tree_selector(pop, key, num_features=None, cells=None, genes=None, normalized=True, normalized_name=None, feature_table=None, ignore=None, clf='extra', n_jobs=1, random_state=None, n_estimators=None, **kwargs):
    ...
    matrix, X, y, y_values = _prep_X_y(pop, key, cells=cells, genes=genes, normalized=normalized, normalized_name=normalized_name, feature_table=feature_table, ignore=ignore, verbose=True, **kwargs)

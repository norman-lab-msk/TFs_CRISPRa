...
def get_cell_phase(pop, gene_list=None, refine=False, threshold=0):
    ...
    ##updated to run on large numbers of cells without large correlation matrix
    normalized_phase_scores_corr = normalized_phase_scores.transpose()
    phase_vectors = pd.DataFrame({'G1-S':[1, 0, 0, 0, 0],
                                  'S':[0, 1, 0, 0, 0],
                                  'G2-M':[0, 0, 1, 0, 0],
                                  'M':[0, 0, 0, 1, 0],
                                  'M-G1':[0, 0, 0, 0, 1]},index=['G1-S','S','G2-M','M','M-G1']).astype('float')
    phase_list = ['G1-S', 'S', 'G2-M', 'M', 'M-G1']
    
    # final scores for each phase are correlation of expression profile with vectors defined above
    cell_cycle_scores = pd.DataFrame()
    for phase,x in phase_vectors.iterrows():
        cell_cycle_scores[phase]=normalized_phase_scores_corr.apply(lambda k: pearsonr(k,x)[0])
